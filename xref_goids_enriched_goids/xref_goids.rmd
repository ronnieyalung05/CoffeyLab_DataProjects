...
# uses `sample_data_with_go` from annotation_retrieval_and_filtering, and `gost_results` (function 2) from gprofiler_enrichment_analysis
```{r}
# one sample
...
df_top <- all_sample_go_filtered$HNPCC_CL$filtered_data[order(all_sample_go_filtered$HNPCC_CL$filtered_data$HNPCC_CL, decreasing = TRUE), ]

df_top <- head(df_top, 50)  # take top N rows
```

```{r}
# filter by expression value cutoff (note, these values are normalized from their original values (log2))
df_cutoff <- all_sample_go_filtered$HNPCC_CL$filtered_data[all_sample_go_filtered$HNPCC_CL$filtered_data$HNPCC_CL >= 5, ]
```

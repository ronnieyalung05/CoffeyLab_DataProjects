...
add_go_annotations <- function(df, uniprot_col = "UniProtID", workers) {
  if (!uniprot_col %in% base::colnames(df)) {
    base::stop(paste("Column '", uniprot_col, "' not found in data frame."))
  }
  
  future::plan(future::multisession, workers = workers)  # <- this line

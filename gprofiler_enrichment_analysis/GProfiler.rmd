...
gost_results <- run_gost_menu_with_top_n(gpr_list)
```
...
```
# TODO: filter top n -> n -> want it so each sample contributes their top n (might get n+20), and then plots themn all on the same heatmap